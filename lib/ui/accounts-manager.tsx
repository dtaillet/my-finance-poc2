...
import { deleteAccounts } from '@/app/accounts/actions';
import type { Account } from '@/lib/data/accounts';
import AccountForm from '@/lib/ui/account-form';
...
export default function AccountsManager({ accounts }: { accounts: Account[] }) {
  ...
  function toggleAll() {
    setSelected((previous) => (previous.size === accounts.length ? new Set() : new Set(accounts.map((account) => account.id))));
  }
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="delete-dialog-title">
          <div className="w-full max-w-md rounded-xl border border-line-2 bg-card p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-muted-foreground-1">Are you sure you want to delete {selected.size} account{selected.size === 1 ? '' : 's'}? This action cannot be undone.</p>
